...
class Ledger_Client():
    ...
    def has_usable_connection_with_device(self):
        try:
            ...
            if e.sw == 0x6d00 or e.sw == 0x6700:
                raise Exception(_("Device is not in BitcoinZ mode")) from e
            if e.sw == 0x6982:
    ...
    def get_xpub(self, bip32_path, xtype):
        self.checkDevice()
        # bip32_path is of the form 44'/177'/1'
...
class LedgerPlugin(HW_PluginBase):
    ...
    def setup_device(self, device_info, wizard, purpose):
        ...
        if client is None:
            ...
            raise Exception("Device is not in BitcoinZ mode")

        client.handler = self.create_handler(wizard)
        client.get_xpub("m/44'/177'", 'standard') # TODO replace by direct derivation once Nano S > 1.1
